...
class TestSaveProjectTransactionalRevisionSnapshot:
    ...
    def test_snapshot_inserted_exactly_once_per_save(self):
        ...
        # Two execute calls: UPDATE project + INSERT project_revisions
        assert conn.execute.call_count == 2
        snapshot_sql = conn.execute.call_args_list[1][0][0]
    ...
    def test_snapshot_summary_is_empty_string(self):
        """The summary field must be an empty string (not None)."""
        from storage import PostgresStorageBackend
        ...
        snapshot_sql = conn.execute.call_args_list[1][0][0]
        # The summary placeholder should be an empty string literal in the SQL
        assert "''" in snapshot_sql
    ...
    def test_two_saves_produce_two_distinct_revision_rows(self):
        ...
        # Each connection should see exactly 2 execute calls (UPDATE + INSERT).
        assert conn1.execute.call_count == 2
        assert conn2.execute.call_count == 2

        rev1 = conn1.execute.call_args_list[1][0][1]["revision"]
        rev2 = conn2.execute.call_args_list[1][0][1]["revision"]
        assert rev1 == 2
        assert rev2 == 3
        assert rev1 != rev2
